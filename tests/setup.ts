...
async function mintTokensOnL1(alternativeToken: string) {
    ...
    const baseTokenMintTx = await testnetToken.mint(await wallet.getAddress(), BigNumber.from("1000000000000000000000000"));
    await baseTokenMintTx.wait();
        
    const altToken = ITestnetErc20TokenFactory.connect(alternativeToken, wallet._signerL1());
    const altTokenMintTx = await altToken.mint(await wallet.getAddress(), BigNumber.from("1000000000000000000000000"));
    ...
    const wethDepositTx = await weth.deposit({value: BigNumber.from("10000000000000000000000")});
