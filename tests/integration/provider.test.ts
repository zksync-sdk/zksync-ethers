...
describe("Provider", () => {
    ...
    describe("#getBytecodeByHash()", () => {
        it("should return the bytecode of a contract", async () => {
            ...
            const testnetPaymasterBytecodeHash = ethers.hexlify(utils.hashBytecode(testnetPaymasterBytecode)); // "0x010000d590477725d953dc54a472de0943121b913d97843dc3e49e4f8519f34d"
            const result = await provider.getBytecodeByHash(testnetPaymasterBytecodeHash);
            expect(result).to.be.deep.equal(Array.from(ethers.getBytes(testnetPaymasterBytecode)));
        });
    });
