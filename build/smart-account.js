...
function checkProvider(signer, operation) {
    ...
    ethers_1.ethers.assert(false, 'missing provider', 'UNSUPPORTED_OPERATION', {
        operation,
    });
}
/**
 ...
 */
class SmartAccount extends ethers_1.AbstractSigner {
    /**
    ...
    constructor(signer, provider) {
        super(provider);
        ethers_1.ethers.defineProperties(this, {
    ...
    async getAllBalances() {
        return await checkProvider(this, 'getAllAccountBalances').getAllAccountBalances(await this.getAddress());
    }
    /**
    ...
    async getDeploymentNonce() {
        return await typechain_1.INonceHolder__factory.connect(utils_1.NONCE_HOLDER_ADDRESS, this).getDeploymentNonce(await this.getAddress());
    }
    /**
    ...
    async signTransaction(tx) {
        ...
        return (0, utils_1.serializeEip712)(populatedTx);
    }
    /**
    ...
    async sendTransaction(tx) {
        return checkProvider(this, 'broadcastTransaction').broadcastTransaction(await this.signTransaction(tx));
    }
    /**
     ...
     */
    signMessage(message) {
        return this.payloadSigner((0, ethers_1.hashMessage)(message), this.secret, this.provider);
    }
    /**
    ...
    async signTypedData(domain, types, value) {
        const populated = await ethers_1.TypedDataEncoder.resolveNames(domain, types, value, async (name) => {
            return ethers_1.ethers.resolveAddress(name, this.provider);
        });
        return this.payloadSigner(ethers_1.TypedDataEncoder.hash(populated.domain, types, populated.value), this.secret, this.provider);
    }
    /**
