...
class Signer extends (0, adapters_1.AdapterL2)(ethers_1.ethers.JsonRpcSigner) {
    ...
    async populateFeeData(transaction) {
        ...
        const { gasLimit, gasPrice, gasPerPubdata } = await (0, utils_1.resolveFeeData)(tx, this.provider, this.providerL2);
        tx.gasLimit = ethers_1.ethers.getBigInt(gasLimit);
        if (!tx.gasPrice && tx.type === 0) {
            tx.gasPrice = ethers_1.ethers.getBigInt(gasPrice);
        }
        else if (!tx.gasPrice && tx.type !== 0) {
            tx.maxFeePerGas = ethers_1.ethers.getBigInt(gasPrice);
