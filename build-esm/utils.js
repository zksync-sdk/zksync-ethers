import { AbiCoder, ethers, resolveProperties, } from 'ethers';
...
export function encodeNativeTokenVaultAssetId(chainId, address) {
 ...
 **/
export async function resolveAssetId(token, ntvContract) {
 ...
 **/
export function encodeNativeTokenVaultTransferData(amount, receiver, token) {
 ...
 */ export function encodeSecondBridgeDataV1(assetId, transferData) {
    const abi = new AbiCoder();
    const data = abi.encode(['bytes32', 'bytes'], [assetId, transferData]);
    return ethers.concat(['0x01', data]);
}
export function encodeNTVAssetId(chainId, address) {
    const abi = new AbiCoder();
    const hex = abi.encode(['uint256', 'address', 'address'], [chainId, L2_NATIVE_TOKEN_VAULT_ADDRESS, address]);
    return ethers.keccak256(hex);
}
export async function ethAssetId(provider) {
    const network = await provider.getNetwork();
    return encodeNTVAssetId(network.chainId, ETH_ADDRESS_IN_CONTRACTS);
}
export function encodeNTVTransferData(amount, receiver, token) {
    return new AbiCoder().encode(['uint256', 'address', 'address'], [amount, receiver, token]);
}
export async function resolveFeeData(tx, provider, providerL2) {
