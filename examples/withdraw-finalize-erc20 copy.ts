...
async function main() {
  const token = '0x';
  const tokenL2 = await l2_provider.l2TokenAddress(token);
  console.log(
    `L2 balance before finalize withdrawal: ${await wallet.getBalance(tokenL2)}`
  );
  console.log(
    `L1 balance before finalize withdrawal: ${await wallet.getBalanceL1(token)}`
  );
  ...
  console.log(
    `L2 balance after finalize withdrawal: ${await wallet.getBalance(tokenL2)}`
  );
  console.log(
    `L1 balance after finalize withdrawal: ${await wallet.getBalanceL1(token)}`
  );
}
